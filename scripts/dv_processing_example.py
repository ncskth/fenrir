...
if __name__ == "__main__":
    ...
    parser.add_argument("--hot_pixel_dir", type=str, default=None)
    parser.add_argument("--no_filters", action="store_true")
    args = parser.parse_args()

    if not args.no_filters and args.hot_pixel_dir is None:
        raise ValueError("`hot_pixel_dir` must be set if filters are used")
    ...
    if not args.no_filters:
        ...
        hot_pixels_left_x = np.load(join(args.hot_pixel_dir, "hot_pixels_left_x.npy"))
        hot_pixels_left_y = np.load(join(args.hot_pixel_dir, "hot_pixels_left_y.npy"))
        hot_pixels_right_x = np.load(join(args.hot_pixel_dir, "hot_pixels_right_x.npy"))
        hot_pixels_right_y = np.load(join(args.hot_pixel_dir, "hot_pixels_right_y.npy"))
